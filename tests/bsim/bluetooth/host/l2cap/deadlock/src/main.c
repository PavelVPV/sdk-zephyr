...
/* Trigger deadlock in flash_sync_mpsl.c */
//#define DEADLOCK_FLASH
/* Trigger deadlock in host. */
#define DEADLOCK_HOST
...
#if defined(DEADLOCK_FLASH)
	/* Trigger timeout in flash_sync_mpsl.c */
	int err;

	/* This deadlock happens because hci_rx_pool is full, we hold the receive callback, which
	 * should release the buffer from the hci_rx_pool, the hci driver is waiting for a free
	 * buffer and flash erase with partial erase enabled is a long operation which partially
	 * relies on mpsl events executed from the same context which hci driver pushes data from
	 * the controller.
	 *
	 * More pages are erased, more hci_rx_pool should be to avoid deadlock.
	 */
	for (int i = 0; i < 2; i++) {
		err = flash_area_erase(flash_area, i * FLASH_PAGE_SIZE, FLASH_PAGE_SIZE);
		if (err) {
			FAIL("Failed to erase flash area (err %d)", err);
		}
	}
#elif defined(DEADLOCK_HOST)
	/* Trigger deadlock in host. */
...
static K_WORK_DEFINE(flash_work, flash_work_handler);
...
static void flash_work_handler(struct k_work *work)
{
	 ...
	 * More pages are erased, more hci_rx_pool should be to avoid deadlock.
	 */
	err = flash_area_erase(flash_area, 0, FLASH_PAGE_SIZE);
	if (err) {
		FAIL("Failed to erase flash area (err %d)", err);
	}
...
static void test_dut_main(void)
{
	...
	err = flash_area_open(FIXED_PARTITION_ID(test_partition), &flash_area);
	if (err) {
		FAIL("Cannot open flash area (%d)", err);
	}
...
#define ADV_WORKQ_NUM_THREADS 1//(CONFIG_BT_BUF_CMD_TX_COUNT * 2)
